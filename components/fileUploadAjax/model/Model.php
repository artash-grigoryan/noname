 ...
 */
    ...
    function __construct($options = false, $initialize = false, $error_messages = null) 
    {
    ...
    protected function set_additional_file_properties($file) {
        $file->delete_url = BASE_PATH.'fileUploadAjax/'.$this->options['actionFile'].'/'
            .'?delete=1&file='.rawurlencode($file->name);
    ...
    public function post($print_response = true) {
        ...
        if (!empty($this->options['fileName'])) {
            $this->deleteForReplace($this->options['fileName']);
        }
    ...
    public function deleteForReplace($file_name) {
        $types = array( '.png', '.jpg', '.jpeg', '.gif');
        $i = 0;
        while(!empty($types[$i]))
        {
            $file_path = $this->get_upload_path($file_name.$types[$i]);
            $success = is_file($file_path) && $file_name[0] !== '.' && unlink($file_path);
            if ($success) {
                foreach($this->options['image_versions'] as $version => $options) {
                    if (!empty($version)) {
