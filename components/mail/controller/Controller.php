...
class MailController  extends Controller  {
    ...
    function sendMailNewPartner($params = array('user_name'=>'Unknown', 'mail'=>'niko_pacha@hotmail.fr', 'token'=>'valid_number'))
    {
        if(!self::getVars('view', false)) {
            $this->_View->disableView();
            $this->_Model->sendMailNewPartner($params['mail'], $params['user_name'], $params['token']);
        }
        else {
            $this->_View->_mail = $params['mail'];
            $this->_View->_token = $params['token'];
